...
backend/
  ...
  app/
    ...
    auth/db.py                 Postgres (psycopg 3) connection pool + schema: users (incl. display name, set at signup) + sessions + cases + rate_limit_hits tables. get_pool()/get_connection()/init_db()/reset_pool(); DDL is idempotent (CREATE/ALTER ... IF NOT EXISTS), run at startup, incl. columns added after a DB was created
...
frontend/
  ...
  src/
    ...
    pages/
      ...
      AuthPage.tsx           login/signup form (mode prop), used at /login and /signup — signup also asks for the user's full name
